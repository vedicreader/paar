...
@rt('/agent/api/info')
def agent_info():
    "The agent-facing contract: env label, mode, policy text, and the names the agent has created so far."
    return _agent_gate() or JSONResponse({'env': _env, 'mode': _agent_mode, 'policy': _POLICY_TEXT,
                                          'agent_names': sorted(_agent_session().layer)})
